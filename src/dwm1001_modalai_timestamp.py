...
class dwm1001_localizer:
    ...
    def __init__(self) :
        ...
        self.sub = rospy.Subscriber ( "/time/offset", Header, self.timeOffsetCb , queue_size=10)
        self.timeoffset = rospy.Time.now()
    ...
    def timeOffsetCb(self , data):
        self.timeoffset = data.stamp
    ...
    def publishTagPositions(self, serialData):
        ...
        if b"POS" in ser_pose_data[0] :
            ...
            ps.header.stamp = self.timeoffset
    ...
    def publishTagPoseKF(self, id_int, id_str, kfPoseData):
        ...
        ps.header.stamp = self.timeoffset
        ps.header.frame_id = "map" # use MAC ID of the Tag as a frame ID for ROS

        if id_int not in self.topics_kf:
            self.topics_kf[id_int] = rospy.Publisher("/mavros/vision_pose/pose", PoseStamped, queue_size=10)
